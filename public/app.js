...
/* ---------- 老師後台 ---------- */
const TEACHER_PASSWORD = 'teacher123'; // 預設密碼，可自行修改
...
async function loadTeacherData() {
  try {
    const [multiData, soloData] = await Promise.all([
      fetchLeaderboard('multi'),
      fetchLeaderboard('solo')
    ]);

    // 合併並去重：同班同學號取最高分
    const studentMap = new Map();
    
    [...multiData, ...soloData].forEach(entry => {
      const key = entry.class + ':' + entry.studentId;
      ...
      // 累加分數和次數
      if (entry.mode === 'multi' || entry.mode === undefined) {
        // 這裡需要判斷模式，暫時用簡單邏輯
        student.multiScore = Math.max(student.multiScore || 0, entry.score);
        student.totalGames = (student.totalGames || 0) + 1;
      } else if (entry.mode === 'solo') {
        student.soloScore = Math.max(student.soloScore || 0, entry.score);
        student.totalGames = (student.totalGames || 0) + 1;
      }
      student.lastPlayed = entry.timestamp;
    });

    teacherData.students = Array.from(studentMap.values());
    renderTeacherTable();
  } catch (e) {
    console.error('載入老師數據失敗:', e);
    toast('載入數據失敗');
  }
}
...
function renderTeacherTable() {
  ...
  const pageData = filtered.slice(start, start + teacherData.pageSize);

  // 渲染表格
  const tbody = $('#studentTableBody');
  tbody.innerHTML = pageData.map((student, i) => {
    const rank = i + 1 + (teacherData.currentPage - 1) * teacherData.pageSize;
    const bestScore = Math.max(student.multiScore || 0, student.soloScore || 0);
    const lastPlayed = student.lastPlayed ? new Date(student.lastPlayed).toLocaleString('zh-HK') : '從未';
    
    return `
      <tr>
        <td>${i + 1 + (teacherData.currentPage - 1) * teacherData.pageSize}</td>
        <td>${student.class}</td>
        <td>${student.studentId}</td>
        <td>${escapeHtml(student.name)}</td>
        <td>${Math.max(student.multiScore || 0, student.soloScore || 0)}</td>
        <td>${student.multiScore || 0}</td>
        <td>${student.soloScore || 0}</td>
        <td>${student.totalGames || 0}</td>
        <td>${lastPlayed}</td>
        <td><button class="action-btn delete-btn" onclick="deleteStudent('${student.class}','${student.studentId}')">刪除</button></td>
      </tr>
    `;
  }).join('');
...
function deleteStudent(className, studentId) {
  if (!confirm(`確定要刪除 ${className} 班 ${studentId} 號的所有記錄？此操作不可復原！`)) return;
  
  // 這裡需要呼叫後端 API 刪除
  // 暫時先從前端移除
  teacherData.students = teacherData.students.filter(s => 
    !(s.class === className && s.studentId === studentId)
  );
  renderTeacherTable();
  toast('已移除（需後端同步才能永久刪除）');
}

function confirmClear(mode) {
  const modeText = mode === 'solo' ? '個人挑戰' : '雙人對戰';
  if (!confirm(`確定要清空所有「${modeText}」排行榜資料？此操作不可復原！`)) return;
  ...
  fetch('/api/leaderboard', {
    ...
    if (r.ok) {
      toast(`${modeText}排行榜已清空`);
      if (teacherData.currentMode === mode) loadTeacherData();
    } else {
      toast('清空失敗');
    }
  });
...
function exportCSV() {
  const { students } = teacherData;
  if (!students.length) { toast('無資料可匯出'); return; }
  
  const headers = ['排名','班別','學號','姓名','最高分','雙人對戰分','個人挑戰分','遊戲次數','最後遊戲時間'];
  const rows = teacherData.students.map((s, i) => {
    const bestScore = Math.max(s.multiScore || 0, s.soloScore || 0);
    const lastPlayed = s.lastPlayed ? new Date(s.lastPlayed).toLocaleString('zh-HK') : '從未';
    return [i+1, s.class, s.studentId, s.name, 
      Math.max(s.multiScore||0, s.soloScore||0), s.multiScore||0, s.soloScore||0, 
      s.totalGames||0, lastPlayed].join(',');
  });
  ...
  a.href = url; a.download = `排行榜_${new Date().toLocaleDateString('zh-HK')}.csv`;
...
$('#nextPage').addEventListener('click', () => {
  ...
  const totalPages = Math.ceil(filtered.length / teacherData.pageSize) || 1;
  if (teacherData.currentPage < totalPages) {
    teacherData.currentPage++;
    renderTeacherTable();
  }
});
...
  
 c o n s o l e . l o g ( ' ? � ? "�? ���UK�? ��d0�I�? ? ) ;  
     }   c a t c h   ( e )   {  
         c o n s o l e . e r r o r ( ' -�\���? ? �? ;RI�? ���N? ? ' ,   e ) ;  
 }  
  
